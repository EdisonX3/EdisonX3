...
uint256 hashGenesisBlock("0x3fe3264d55102f59879161e15abb35e39f860d3c647e6e64ed76b52146074708");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EdisonX3: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EdisonX3 Signed Message:\n";
...
{
	int64 nSubsidy = 1922 * COIN;
	if(nHeight < 17275681) // stops generation of more coins
	{
		nSubsidy = 45 * COIN;
	}
	if(nHeight < 12019680)
	...
	if(nHeight == 1)
	{
		nSubsidy = 2000000 * COIN; // 0.2% for bounties and giveaways.
	}
	return nSubsidy + nFees;
}


static const int64 nTargetTimespan = 30 * 60;	// EdisonX3: 0.35 day
static const int64 nTargetSpacing = 30 * 60;						// EdisonX3: 15 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 30 * 60; // 60 Minutes
static const int64 nTargetSpacingRe = 30 * 60; // EdisonX3: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // EdisonX3: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal edisonx3 miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EdisonX3", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xac2624b647e464937cfe139c5e4775818a0478cd7aefaf374a6d5aa48220fdbc");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Banks to legal marijuana businesses: We're open";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392783525;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1303031;

        if (fTestNet)
        {
            block.nTime    = 1392456407;
            block.nNonce   = 3685030;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9adff07bae9f7f52c1432ba5603a1b1aeeca45a3faa3d6c492f4cb3b3eb9be3d"));
...
{
	return false;
	//commenting out this code until we properly implement EDISON alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // EdisonX3: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "edisonalert")
    {
        CAlert alert;
        vRecv >> alert;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // EdisonX3d: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
