...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EdisonX3 " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    // {"edisonx3pool.org", "dnsseed.edisonx3pool.org"},
...
void ThreadOpenAddedConnections(void* parg)
{
    ...
    try
    {
        vnThreadsRunning[THREAD_ADDEDISONONNECTIONS]++;
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EdisonX3 is probably already running."), addrBind.ToString().c_str());
        else
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_ADDEDISONONNECTIONS] > 0) printf("ThreadOpenAddedConnections still running\n");
